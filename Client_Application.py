#############################################################################
# this is an original source code written by Mkandawire Samuel 2024         #
#############################################################################
import cv2
import numpy as np
...
# Load the model
model = load_model("./hand_model.h5", compile=False)
...
def run_script():
    ...
    subprocess.run(["python", "CollectImage.py"])
...
tk.Button(root, text="Train Model", image=dataset_icon, border=0, command=run_script).place(x=650, y=10)

# create a button to open the training model
model_icon = tk.PhotoImage(file="./resources/train_model.png")
tk.Button(root, text="Train Model", image=model_icon, border=0, command=run_script).place(x=650, y=100)
